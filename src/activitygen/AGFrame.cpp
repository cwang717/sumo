...
void AGFrame::fillOptions() {
    ...
    oc.addDescription("net-file", TL("Input"), TL("Use FILE as SUMO-network to create trips for"));

    oc.doRegister("stat-file", 's', new Option_FileName());
    oc.addDescription("stat-file", TL("Input"), TL("Loads the SUMO-statistics FILE"));
    ...
    oc.addDescription("begin", TL("Time"), TL("Sets the time of beginning of the simulation during the first day (in seconds)"));

    oc.doRegister("end", 'e', new Option_Integer(0));
    oc.addDescription("end", TL("Time"), TL("Sets the time of ending of the simulation during the last day (in seconds)"));

    oc.doRegister("duration-d", new Option_Integer(1));
    oc.addDescription("duration-d", TL("Time"), TL("Sets the duration of the simulation in days"));
}
